...
module.exports.config = {
  ...
  usages: "tx tài/xỉu, b3gn, b2gn, cuoctong, cuocso, số tiền",
  cooldowns: 10
};
...
// Hệ số thưởng cuoctong theo tổng
function getRATE(tong) {
  if (tong === 4 || tong === 17) return 10;
  if (tong === 5 || tong === 16) return 8;
  if (tong === 6 || tong === 15) return 6;
  if (tong === 7 || tong === 14) return 5;
  if (tong === 8 || tong === 13) return 4;
  if (tong === 9 || tong === 12) return 3;
  if (tong === 10 || tong === 11) return 2;
  return 0;
}
...
module.exports.run = async function ({ event, api, Currencies, Users, args }) {
  try {
    ...
    if (choose === 'cuocso') {
      ...
      const rate = getRATE(tong);
      const chance = Math.min(0.3 + (rate / 10) * 0.7, 0.95); // Xác suất thắng
      if (total === tong && Math.random() < chance) {
        result = 'win';
        mn = bet * rate;
      } else {
        result = 'lose';

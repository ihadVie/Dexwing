...
module.exports.run = async function({ api, event, Users }) {
    ...
    const dbPath = path.join(__dirname, "../../includes/data.sqlite");
    ...
    db.all("SELECT * FROM users", [], async (err, rows) => {
        ...
        for (let user of rows) {
            let data;
            try { data = JSON.parse(user.data); } catch(e){ continue; }
            if (!data) continue;
            ...
            const totalFish = data.fishHistory ? data.fishHistory.length : 0;
            const totalFishMoney = data.fishHistory ? data.fishHistory.reduce((sum, f) => sum + (f.price || 0), 0) : 0;
            const rareFish = data.fishHistory && data.fishHistory.length > 0
              ? data.fishHistory.reduce((prev, curr) => {
